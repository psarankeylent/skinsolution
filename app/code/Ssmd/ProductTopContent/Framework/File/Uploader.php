 ...
 */
namespace Ssmd\ProductTopContent\Framework\File;
...
class Uploader
{
    ...
    private function _setUploadFileId($fileId)
    {
        if (is_array($fileId)) {
            ...
            $file = array_filter(preg_split('/(\[|\])/', $fileId), static function($var){return $var !== "";} );

            /*$tmpFileId = str_replace('[', ']', $fileId);
            $tmpFileId = str_replace(']]', ']', $tmpFileId);

            $file = explode(']', $tmpFileId);*/
            ...
            if (is_array($file) && count($file) > 0 && !empty($file[0]) && !empty($file[1])) {
                ...
                foreach ($fileAttributes as $attributeName => $attributeValue) {
                    ...
                    foreach ($file as $fileDepth) {
                        $finalAttributeValue = $finalAttributeValue[$fileDepth];
                    }
                    $tmpVar[$attributeName] = $finalAttributeValue;//$attributeValue[$file[1]];
                }
