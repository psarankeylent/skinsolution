...
class Data extends AbstractHelper
{
    ...
    public function orderItemReport($key, $value)
    {
        ...
        $collection->getSelect()
            ->joinLeft(array('so'=> 'sales_order'),
                'main_table.order_id = so.entity_id', array('so.increment_id','so.entity_id','so.status','so.customer_email','so.discount_amount as order_level_discount',"CONCAT(so.customer_firstname, ' ', so.customer_lastname) as customer_name"))
            ->joinLeft( array('soa'=> 'sales_order_address'),
            ...
            foreach ($d_filters as $filterKey => $filterValue) {
                ...
                if($filterKey == 'ship_date' && ($filterValue->from != "" && $filterValue->to != "") )
                {
                    if(is_object($filterValue))
                    {
                        $fromDate = $filterValue->from;
                        $toDate   = $filterValue->to;
                        $collection->addFieldToFilter('soship.ship_date', ['from' => $fromDate, 'to'=>$toDate]);
                    }
                }
    ...
    public function createCsvForOrderItemReport($collection, $createdDate, $downloadReportId,$adminName, $adminEmail)
    {
        try{
            ...
            $columns = ['Order Number','Customer Name','Customer Email','Sku','Item Name','Qty','Item Price','Item Cost','Item Level Discount','Order Level Discount','Item Tax','Item Total','Order Status','Shipping State','Order Date','Invoice Date','Ship Date','Order Type','Warehouse','Shipping Cost',];
    ...
    public function createCsvForBatchListReport($collection, $createdDate,$downloadRportsId,$adminName, $adminEmail)
    {
        try{
            ...
            // Send email
            $this->sendEmail($adminName, $adminEmail, $filepath,$downloadRportsId);
            return true;
    ...
    public function impersonationReport($key, $value)
    {
        ...
        if(!empty($filters))
        {
            ...
            foreach ($d_filters as $filterKey => $filterValue) {
                ...
                elseif($filterKey == 'username' && $filterValue != "")
                {
                ...
                elseif($filterKey == 'customer_email' && $filterValue != "")
                {
                    $collection->addFieldToFilter('customer_email', $filterValue);
                }             
            }
        }
    ...
    public function storeCreditReport($key, $value)
    {
        ...
        if(!empty($filters))
        {
            ...
            foreach ($d_filters as $filterKey => $filterValue) {
                ...
                elseif($filterKey == 'amount' && $filterValue != "")
                {
                    $collection->addFieldToFilter('amount', ['like' => '%'.$filterValue.'%']);
                }
                else
                {
                    if(isset($filterValue) && $filterValue != "")
    ...
    public function subscriptionReport($key, $value)
    {
        ...
        if(!empty($filters))
        {
            ...
            foreach ($d_filters as $filterKey => $filterValue) {
                ...
                elseif($filterKey == 'tax_amount' && $filterValue != "")
                {
                    $collection->addFieldToFilter('tax_amount', ['like' => '%'.$filterValue.'%']);
                }
                elseif($filterKey == 'name' && $filterValue != "")
                {
                    $collection->addFieldToFilter('name', ['like' => '%'.$filterValue.'%']);
                }
                else
                {
                    if(isset($filterValue) && $filterValue != "")
    ...
    public function ccExpirationReport($key, $value)
    {
        ...
        if(!empty($filters))
        {
            ...
            foreach ($d_filters as $filterKey => $filterValue) {
                ...
                elseif($filterKey == 'firstname' && $filterValue != "")
                {
                    $collection->addFieldToFilter('firstname', ['like' => '%'.$filterValue.'%']);
    ...
    public function prescriptionExpirationReport($key, $value)
    {
        ...
        if(!empty($filters))
        {
            ...
            foreach ($d_filters as $filterKey => $filterValue) {
                ...
                elseif($filterKey == 'name' && $filterValue != "")
                {
                    $collection->addFieldToFilter('firstname', $filterValue);
                }
                else
                {
                    $collection->addFieldToFilter($filterKey, $filterValue);
    ...
    public function sendEmail($adminName="", $adminEmail="", $file_path=null,$downloadReportId=0)
    {
        ...
        try {
            ...
            $logger->info('sender '.json_encode($sender));
    ...
    public function sendEmailPHPWay($adminName="", $adminEmail="", $file_path=null,$downloadReportId=0)
    {
        $email = (new Email())
            ->from(new Address('noreply@your-domain-here.com', 'Mailtrap Test'))
            //->replyTo(new Address('reply@your-domain-here.com'))
            ->to(new Address('babariyahiteshb@gmail.com', 'Hitesh'))
        ...
        try {
            $apiKey = "4ccd5725424da284851225f30b740257";
    ...
    /*public function sendEmailZend($adminName="", $adminEmail="", $file_path=null,$downloadReportId=0)
    {
        ...
        $options   = new SmtpOptions(array(
            'name'              => 'live.smtp.mailtrap.io',
            'host'              => 'live.smtp.mailtrap.io',
            'port'              => 587
            'connection_class'  => 'login',
            'connection_config' => array(
                'username' => 'api',
                'password' => '4ccd5725424da284851225f30b740257',
            ),
        ));
    ...
    public function getBaseCost($product_id)
    {
        $objectManager  = \Magento\Framework\App\ObjectManager::getInstance();
