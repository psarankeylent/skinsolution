...
define([
...
], function (_, utils, layout, Collection, $t, $, modal) {
    'use strict';
    ...
    return Collection.extend({
        ...
        downloadReportBtn: function () {    // Below functions created by hitesh
            //console.log('hitesh');
            var namespace = this.ns;
            /*if(namespace == 'renewal_declinedsubscriptionsm1_report_listing')   // Hide filter button for this grid.
            {
                $('.data-grid-filters-action-wrap .action-default').hide();
            }*/

            var showDownloadReportBtn = false;
            if(
                namespace == 'renewal_order_report_listing' ||
                namespace == 'renewal_order_item_report_listing' ||
                namespace == 'customreports_batchlist_report_listing' ||
                namespace == 'customreports_impersonation_report_listing' ||
                namespace == 'customreports_authunsettledtrans_report_listing' ||
                namespace == 'customreports_storecreditusage_report_listing' ||
                namespace == 'customreports_reconciliation_invoice_report_listing'||
                namespace == 'renewal_storecredit_report_listing' ||
                namespace == 'subscription_report_listing' ||
                namespace == 'renewal_declinedsubscriptions_report_listing' ||
                namespace == 'renewal_declinedsubscriptionsm1_report_listing' || 
                namespace == 'reports_cc_listing' ||
                namespace == 'renewal_expiration_report_listing'
            )
            {
                showDownloadReportBtn = true;
            }
            return showDownloadReportBtn;
        },
        
        downloadReport: function () {    // Function added by Hitesh
            
            var adminUrl = window.adminUrl;
            var adminBaseUrl = adminUrl.replace(/admin\/$/, '');
            console.log(adminBaseUrl);
            
            var fullUrl      = adminBaseUrl+'downloadreports/download/index';
            
            var names = this.ns;
            var downloadFilters = this.filters;
            ...
            $.ajax({
                ...
                type: "POST",
                showLoader: true,
                data:{'namespace':names, 'downloadFilters':downloadFilters, 'form_key':formKey},
                success: function (data, status, xhr) {                        
                    ...
                    $("#popup-modal-main").text(data); 
                    $("#popup-modal-main").modal(options).modal('openModal');
