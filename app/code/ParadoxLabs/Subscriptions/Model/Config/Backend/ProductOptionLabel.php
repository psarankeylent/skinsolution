...
/**
 * ProductOptionLabel Class
 */
class ProductOptionLabel extends \Magento\Framework\App\Config\Value
{
    /**
    ...
    public function afterSave()
    {
        if ($this->isValueChanged()) {
            $this->updateSubscriptionOptionLabels();
        }
     ...
     * @return void
     */
    protected function updateSubscriptionOptionLabels()
    {
         ...
         *
         * UPDATE `catalog_product_option_title` AS `main_table`
         *  INNER JOIN `paradoxlabs_subscription_product_interval` AS `interval`
         *   ON main_table.option_id=interval.option_id
         *  SET `main_table`.`title` = 'Y'
         *  WHERE (main_table.title='X')
         */
        ...
        $select = $db->select()
            ->joinInner(
                ...
                'main_table.option_id=interval.option_id',
                [
                    'title' => new \Zend_Db_Expr($db->quoteInto('?', $this->getValue())),
                ]
            )
            ->where('main_table.title=?', $this->getOldValue());

        // Create update from select
        $query = $db->updateFromSelect(
            $select,
            [
                'main_table' => $db->getTableName('catalog_product_option_title'),
            ]
        );
        ...
        $this->helper->log(
            'subscriptions',
            __(
                'Updated %1 custom option labels from "%2" to "%3".',
                $result->rowCount(),
                $this->getOldValue(),
                $this->getValue()
            )
