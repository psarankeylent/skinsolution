...
class Discount extends \Magento\SalesRule\Model\Quote\Discount
{
    ...
    )
    {
		...
		$writer = new \Zend\Log\Writer\Stream(BP . '/var/log/impersonation_logger.log');
$logger = new \Zend\Log\Logger();
$logger->addWriter($writer);
$logger->info($item->getId() . ' - ' . $item->getSku() . ' - ' . $item->getPriceType());
$logger->info($item->getIsSuperMode() ? "True" : "False");
$logger->info($item->getProduct()->getIsSuperMode() ?"product super mode true": "product super mode false");	
$logger->info($item->getOriginalCustomPrice() . ' --- original custom price');
