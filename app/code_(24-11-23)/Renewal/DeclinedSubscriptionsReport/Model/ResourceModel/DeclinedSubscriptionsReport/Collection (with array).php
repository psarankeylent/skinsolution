...
class Collection extends AbstractCollection
{
    ...
    {        
        ...
        $subLogCollection  = $objectManager->get('ParadoxLabs\Subscriptions\Model\LogFactory')->create()->getCollection();
        ...
        foreach ($collection as $key => $value) {
            $subscriptionId = $value->getData('entity_id');
            $subLogCollection  = $objectManager->get('ParadoxLabs\Subscriptions\Model\LogFactory')->create()->getCollection();
            $subLogCollection->addFieldToFilter('subscription_id', $subscriptionId);
            $subLogCollection->addFieldToFilter('status', 'payment_failed');
            $subLogCollection->getSelect()->group('subscription_id')->order('subscription_id DESC');
            //$subLogCollection->setOrder('subscription_id', 'DESC');
            $dt = $subLogCollection->getFirstItem();
            $dataArr[$key]['logData'] = $dt->getData();
        }
        ...
        $collection->getSelect()
                            ...
                            ->order('paradoxlabs_subscription_log.subscription_id DESC'); // Fetching latest one.
    
       
       
    
        $collection->addFieldToFilter('paradoxlabs_subscription_log.status', ['eq' => 'payment_failed']);
        //$collection->setOrder('paradoxlabs_subscription_log.subscription_id', 'ASC');
