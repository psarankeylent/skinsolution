    ...
    {        
        ...
        if(isset($filters))
        {
            ...
            if(isset($filters['updated_at']))
            {
                ...
                // Finding Latest Record when filters applied.
                $collection->getSelect()->group('paradoxlabs_subscription_log.subscription_id')
                            ->where('log_id IN((SELECT max(log_id) FROM `paradoxlabs_subscription_log` group by paradoxlabs_subscription_log.subscription_id))');
            }
