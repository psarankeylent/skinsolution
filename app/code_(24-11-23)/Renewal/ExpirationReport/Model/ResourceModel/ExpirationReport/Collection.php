    ...
    public function addFieldToFilter($field, $condition = null)
    {
        if ($field === 'name') {
            ...
            if (strpos(trim($condition), ' ') !== false) {      // If fname & lname searches
                ...
                $query = $this->getSelect()->where('`customer_entity`.`firstname` like "%'.$conditionArr[0].'%" AND `customer_entity`.`lastname` like "%'.$conditionArr[1].'%"');
            ...
            else  // If only firstname/lastname searches
            {
               /* $conditionSql = $this->_getConditionSql(
                    'firstname',  // means $field. here fname+lname
                    $condition   // means value like '%yem kotina%'
                );
                */
                $query = $this->getSelect()->where('`customer_entity`.`firstname` like "%'.$condition.'%" OR `customer_entity`.`lastname` like "%'.$condition.'%"');
            }
